...
[svmStruct, eigVectors, meanMatrix] = trainClassifier_customFcn(dyskinetic_clip_list, normal_clip_list, sampleLength, overlapLength,@averageLowerHalfFeatureVector,1,num_retain);
...
lowerHalf_grades = [2,0,2,2,2,2, 0, 2,0,1,0,0,0,3];
true_labels = lowerHalf_grades > 0;
dyskinetic_patients = find(lowerHalf_grades >0);
normal_patients = find (lowerHalf_grades == 0);
num_retain = 3;
...
for i = 1:length(patients)
    [labels{i}, clip_indices{i},lower_dims{i}] = classifyClips_customFcn_pca(svmStruct,eigVectors, meanMatrix,clip_lists{i}, sampleLength, overlapLength, @averageLowerHalfFeatureVector); 
...
grc = genRankCorr(grades, lowerHalf_grades)
