...
for i = 1:length(vars)
    ...
    elseif ((~isempty(strfind(vars{i}, 'afterMed'))|| ~isempty(strfind(vars{i}, 'afterMed'))) && ~isempty(strfind(vars{i}, 'sit')) ...
            && (~isempty(strfind(vars{i}, 'handopen'))|| ~isempty(strfind(vars{i}, 'handopen'))) )
...
figure();
plot ([0,1, 1.5, 3.5, 4.5, 5.5], recordScores, '--.','Color', [0.7 0.7 0.7], 'MarkerSize', 20, 'MarkerEdgeColor', 'b');
